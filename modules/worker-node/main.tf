...
#Will create: 
#1. Egress for Worker Nodes to Kubernetes Controller
#2. Ingress from Kubernetes Controller to Worker Nodes
...
resource "aws_security_group_rule" "worker_outbound_http" {
  type              = "egress"
  from_port         = 80
  to_port           = 80
  protocol          = "tcp"
...
# Allow outgoing connectivity
resource "aws_security_group_rule" "worker_https_outbound" {
  type              = "egress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
